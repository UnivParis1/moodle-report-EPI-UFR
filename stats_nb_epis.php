			...
			if (!empty($typestat)) {
				if (!empty($ufrs)) {
			        ...
			        foreach( $ufrs as $i=>$ufr) {
						...
						$listelicence= $DB->get_record_sql($SELECT, array('%/'.$id_licence.'/%','%/'.$id_licence));
				        if (empty($listelicence->nb)) $listelicence->nb = 0;
			        	$listemaster= $DB->get_record_sql($SELECT, array('%/'.$id_master.'/%','%/'.$id_master));
				        if (empty($listemaster->nb)) $listemaster->nb = 0;
				        $listedoctorat= $DB->get_record_sql($SELECT, array('%/'.$id_doctorat.'/%','%/'.$id_doctorat));
				        if (empty($listedoctorat->nb)) $listedoctorat->nb = 0;
				        $listetotal = $DB->get_record_sql($SELECT, array('%/'. $ufr->id .'/%','%/'. $ufr->id )); 
				        if (empty($listetotal->nb)) $listetotal->nb = 0;
				        $data = array('<a href="view.php?id='.$ufr->id.'">'.$ufr->name.'</a>',
				    					$listelicence->nb,
				    					$listemaster->nb,
				    					$listedoctorat->nb ,
				    					$listetotal->nb - $listelicence->nb - $listemaster->nb - $listedoctorat->nb,
				    					$listetotal->nb);
						$datas[] = $data;
						$nb_licence += $data[1];
						$nb_master  += $data[2];
			    ...
			    <script type="text/javascript">
			      ...
			      function drawChart() {
				    ...
				    $array_diplome = array();
					$array_diplome[]= array(get_string($typestat,'local_up1reportepiufr'),$nbtotal);
